...
import { JobOrderFormData, Note, QuotationItem, Attachment, User } from '@/types/joborders';
...
export default function JobOrdersCreate({ users }: Props) {
  const { errors } = usePage<{ errors: any }>().props;
  const [ notes, setNotes ] = useState<Note[]>([ { problem_description: '', services_rendered: '' } ]);
  const [ quotation, setQuotation ] = useState<QuotationItem[]>([ { product_name: '', unit: '', qty: 1, price: 0, total: 0 } ]);
  const [ attachments, setAttachments ] = useState<Attachment[]>([ { name: '', file_path: '' } ]);
  ...
  const { data, setData, post, processing, errors: formErrors } = useForm({
    ...
    engineer_supervisor_id: undefined,
    company_manager_id: undefined,
    engineer_signature: '',
    engineer_supervisor_signature: '',
    company_manager_signature: '',
    engineer_approved: false,
    engineer_supervisor_approved: false,
    company_manager_approved: false,
  });
  ...
  const updateAttachment = (index: number, field: keyof Attachment, value: string) => {
    ...
    newAttachments[ index ][ field ] = value as never;
    setAttachments(newAttachments);
    setData('attachments', newAttachments);
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end space-x-4">
                ...
                <Button type="submit" disabled={processing}>
                  {processing ? 'Creating...' : 'Create Job Order'}
                </Button>
              </div>
            </form>
          </CardContent>
